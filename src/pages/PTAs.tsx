...
export function PTAs() {
  ...
  const types = useMemo(
    () => ["All", ...Array.from(new Set(database.pta_organizations.map((p) => p.pta_type).filter(Boolean)))],
    []
  );
  ...
  const results = useMemo(() => {
    ...
    return database.pta_organizations.filter((pta) => {
      ...
      const campus = campusName
        ...
        : undefined;
      const haystack = `${pta.name || ""} ${pta.internal_pta_id} ${campus?.name || ""}`.toLowerCase();
      return (!q || haystack.includes(q)) && (type === "All" || pta.pta_type === type);
    });
  }, [query, type]);
  ...
  return (
    <section className="section">
      <div className="container">
        ...
        <h1>PTAs</h1>
        <p className="page-intro">Browse PTA and PTSA organizations represented in the directory.</p>
        ...
        <div className="card-grid">
          {results.map((pta) => <PTACard key={pta.pta_id} pta={pta} />)}
        </div>
      </div>
    </section>
  );
